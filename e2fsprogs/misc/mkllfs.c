...
const char * program_name = "mkllfs";
...
static int llfs_write_inode_table(ext2_filsys fs, ext2_ino_t ino)
{
	struct ext2_inode inode;
	struct ext2_inode ifile_inode;
	unsigned long off, blk, offset, block_ext2, block_llfs;
	int i, retval;
	char *ptr;
	char buffer[EXT2_BLOCK_SIZE(fs->super)];
	/* read ext2 */
	ext2fs_read_inode(fs, ino, &inode);
	ext2fs_read_inode(fs, 12, &ifile_inode);

	/* write llfs */
	// TODO: compute offset and get blocknr from ifile_inode, etc.
	
	off = (ino - 1) * EXT2_INODE_SIZE(fs->super);
	blk = off >> EXT2_BLOCK_SIZE_BITS(fs->super);
	blk++;
	offset = off & (EXT2_BLOCK_SIZE(fs->super) - 1);
	//for (i=0; i < EXT2_N_BLOCKS; i++) {
		//block_ext2 = inode.i_block[i];
	if (blk > EXT2_N_BLOCKS)
		exit(1);
	block_llfs = ifile_inode.i_block[blk];
	
	if (ino == 12) {
		for (i = 0; i < LLFS_MAX_CLONES; i++)
			fs->super->s_ifile_inode_block[i] = 0;
		fs->super->s_ifile_inode_block[0] = block_llfs;
		fs->super->s_ifile_inode_block[10] = block_llfs;
	}
	retval = io_channel_read_blk(fs->io, block_llfs, 1,
				     buffer);
	if (retval)
		exit(1);
	ptr = (char *)&inode;
	memcpy((char *) buffer + (unsigned) offset,
		ptr, EXT2_INODE_SIZE(fs->super));

	retval = io_channel_write_blk(fs->io, block_llfs, 1,
				      buffer);
/*
	if (fs->icache->buffer_blk != block_llfs) {
		retval = io_channel_read_blk(fs->io, block_llfs, 1,
					     fs->icache->buffer);
		if (retval)
			exit(1);
		fs->icache->buffer_blk = block_llfs;
	}
	ptr = (char *)inode;
...
static void llfs_create_special_file(ext2_filsys fs, char *filename, int size, char *buf_arg)
{
	ext2_file_t     e2_file;
	ext2_ino_t      newfile;
	errcode_t       retval;
	...
	retval = ext2fs_new_inode(fs, EXT2_ROOT_INO, 010600, 0, &newfile);
	//if (newfile == 13) {
	//	//for (i = ; i<30; i++)
	//	buf[0] = 15;    // 1-xx 
	//	buf[16] = 32;   // 134
	//	buf[18] = 124;   // 147 - 151 
	//	buf[384] = 127;  // 3073 - 3077 (?)
	//}
	if (retval) {
	...
	if (ext2fs_test_inode_bitmap(fs->inode_map, newfile))
		com_err("new inode", 0, "Warning: inode already set");
	...
	if (retval) {
		com_err("new inode", retval, "while creating inode %u", newfile);
		exit(1);
	}
	retval = ext2fs_link(fs, EXT2_ROOT_INO, filename, newfile, 
			     EXT2_FT_REG_FILE);
	if (retval) {
		com_err("ext2fs_new_inode", retval, 0);
		exit(1);
	...
	//if (newfile == 12) {
	//	int used_blocks = 14; // can be wrong
	//	struct llfs_group_desc *lgd = malloc(sizeof(struct llfs_group_desc));
	//	lgd->bg_free_inodes_count = 65535 - 15;
	//	int block_size = EXT2_BLOCK_SIZE(fs->super);
	//	lgd->bg_free_blocks_count = block_size * 8 - used_blocks;
	//	memcpy(buf + block_size * 6, lgd, sizeof(struct llfs_group_desc));
	//}
	retval = ext2fs_file_write(e2_file, buf, size, &written);
	if (retval)
		goto fail;
	retval = ext2fs_file_close(e2_file);
	
	//retval = llfs_write_new_inode(fs, newfile, &inode);
	retval = llfs_write_inode_table(fs, newfile);
	if (newfile == 12) {
	    // write root dir and lost and found
	    retval = llfs_write_inode_table(fs, 2);
	    retval = llfs_write_inode_table(fs, 11);
	}
	if (retval) {
		com_err("llfs new inode", retval, "while creating inode %u", newfile);
		exit(1);
	}
	return;
...
static void llfs_update_bitmap(ext2_filsys fs) {
	ext2_file_t     e2_file;
	errcode_t       retval;
	int inos[5] = {2, 11, 12, 13, 14};
	int blk;
	int i;
	struct ext2_inode inode;
	int block_size = EXT2_BLOCK_SIZE(fs->super);
	char	    buf[block_size * 7];
	char	    ifile_buf[block_size * (6 + fs->desc_blocks)];
	unsigned int    written;
	...
	for (i = 0; i < 5; i++) {
		int ino = inos[i];
		ext2fs_read_inode(fs, ino, &inode);
		__u32 ind_buf[block_size / 4];
		//ext2fs_get_blocks(fs, ino, &blocks);
		for (blk = 0; blk < inode.i_size / block_size + 3; blk++) {
			int block;
			if (blk > 12) {
				ext2fs_read_ind_block(fs, inode.i_block[12], ind_buf);
				block = ind_buf[blk - 13];
			} else
				block = inode.i_block[blk];
			if (block != 0) {
				used_blocks++;
				block = block - fs->super->s_first_data_block;
	...
	/* group desc block 153 */ //should be there
	//int db;
        //for (db = 153; db < 153 + fs->desc_blocks + 1; db++) {
        //        used_blocks++;
        //        //int index = (int)(153 - 1) / 8;
        //        //int bit = 1 << ((153 -1) % 8);
        //        int index = (int)(db - 1) / 8;
        //        int bit = 1 << ((db - 1) % 8);
        //        buf[index] |= bit;
        //}
	/* write bitmap */
	retval = ext2fs_file_open(fs, 14, EXT2_FILE_WRITE, &e2_file);
	if (retval)
		exit(retval);
	retval = ext2fs_file_write(e2_file, buf, block_size, &written);
	if (retval)
		exit(retval);
	retval = ext2fs_file_close(e2_file);

	/* write group desc */
	for (i = 0; i < block_size * (6 + fs->desc_blocks); i++)
		ifile_buf[i] = 0;
	struct ext2_group_desc lgd;// = malloc(sizeof(struct llfs_group_desc));
	lgd.bg_free_inodes_count = fs->super->s_inodes_per_group - 15;
	lgd.bg_free_blocks_count = block_size * 8 - used_blocks;
	memcpy(ifile_buf + (0 * sizeof(struct ext2_group_desc)), &lgd, sizeof(struct ext2_group_desc));
	for (i = 1; i < fs->group_desc_count; i++) {
		lgd.bg_free_inodes_count = fs->super->s_inodes_per_group;
		int numblocks;
		if (i == fs->group_desc_count - 1) {
			...
			numblocks = fs->super->s_blocks_per_group;
		lgd.bg_free_blocks_count = numblocks;
		memcpy(ifile_buf + (i * sizeof(struct ext2_group_desc)), &lgd, sizeof(struct ext2_group_desc));
	}

	retval = ext2fs_file_open(fs, 12, EXT2_FILE_WRITE, &e2_file);
	__u64 ret_pos = 0;
	retval = ext2fs_file_llseek(e2_file, (__u64)block_size * 6, EXT2_SEEK_SET, &ret_pos);
	if (retval)
		exit(retval);

	if (retval)
		exit(retval);
	retval = ext2fs_file_write(e2_file, ifile_buf, block_size * (fs->desc_blocks), &written);
	if (retval)
		exit(retval);
	retval = ext2fs_file_close(e2_file);
	retval = llfs_write_inode_table(fs, 12);
}
...
int main (int argc, char *argv[])
{
	...
	if (super_only) {
		...
		int block_size = EXT2_BLOCK_SIZE(fs->super);
		printf("fs->desc_blocks: %i\n", fs->desc_blocks);
		llfs_create_special_file(fs, ".ifile", block_size * (7 + fs->desc_blocks), NULL);
		llfs_create_special_file(fs, ".config", block_size, NULL);
		llfs_create_special_file(fs, ".bitmap", block_size, NULL);
		llfs_update_bitmap(fs);
